...
- A full `/seo audit` (2026-08-23, `claude-seo` plugin, 10 parallel subagents) found and 3 critical items were fixed the same day: (1) added `Restaurant` and `FAQPage` JSON-LD to `index.html` — the site had zero structured data anywhere; (2) converted the hero photo from a JS-only `.ph-image` background-fill `<div>` to a real `<img fetchpriority="high">` with native `width`/`height`/`alt`, so it's indexable by Google Images and visible to the browser's preload scanner (the other `.ph-image` blocks — gallery, ingredients, story — stay JS-lazy-loaded by design, since they're below the fold); (3) compacted the mobile `.cookie-banner` (shorter copy, tighter padding, `min-height:44px` buttons instead of full-width stacked ones) after DOM measurement confirmed it fully covered both hero CTA buttons on a 375×812 viewport on first load. Remaining findings from that audit (OG/Twitter meta, image compression, canonical tags, GEO/llms.txt) are open — not yet actioned. The domain is confirmed as `pinocchiobaja.hu`; `sitemap.xml`, `robots.txt`, and the privacy-policy intro now reference it (2026-08-24), but `og:url`/canonical tags still need it once those are added.
